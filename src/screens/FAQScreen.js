...
class FAQScreen extends React.Component {
   ...
   onShouldStartLoadWithRequest = (event) => {
    return this.onLinkCheck(event.url, PlatformUtils.isIOS());
  }

   onLoadStart = (event) => {
    return this.onLinkCheck(event.nativeEvent.url, PlatformUtils.isAndroid());
  }

   onLinkCheck = (urlLink, isNeedToSendProps) => {
    const {onPressLink, url} = this.props;
    if (urlLink !== url && onPressLink && isNeedToSendProps) {
